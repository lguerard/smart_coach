...
FR_SYSTEM_PROMPT = (
    ...
    "weekly_goal_min, .vo2max/fitness_age (contexte uniquement, cite "
    "seulement si pertinent -- n'invente jamais une tendance sur un "
    "seul jour), .menstrual_cycle_phase si le "
...
EN_SYSTEM_PROMPT = (
    ...
    ".stress_avg_level/max_level, .respiration_avg_waking/avg_sleep, "
    ".spo2_average, .intensity_moderate_min/vigorous_min/"
    "weekly_goal_min, .vo2max/fitness_age (context only, mention only "
    "if relevant -- never invent a trend from a single day), "
